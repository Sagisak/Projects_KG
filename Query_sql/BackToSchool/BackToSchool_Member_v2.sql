...
where rfm.Status  not in ('Trader', 'Inactive') )


SELECT
gm.MyValueId,
min(gm.CustomerName) as 'Customer Name',
min(gm.Phone) as 'Nomor Telepon',
min(gm.EmailAddress) as 'Email',
sum(gt.Jumlah_Transaksi) as 'Jumlah Transaksi',
sum(gt.Jumlah_visits) as 'Jumlah Visits',
sum(gt.Total_Sales) as 'Total Sales',
sum(gt.Average_sales) as 'Average Sales'
FROM GettingTrader gm
inner join GettingTransaction gt on gm.MyValueId = gt.MyValueId and gm.DataAreaId = gt.DataareaID
group by gm.MyValueId 
having sum(gt.Total_Sales) >= 500000
